...
const User1 = () => {
  const [customizations, setCustomizations] = useState(() => {
    const savedState = localStorage.getItem("customizations");
    return savedState
  ...
  const [editableStr, setEditableStr] = useState(customizations.name);

  useEffect(() => {
    localStorage.setItem("customizations", JSON.stringify(customizations));
  }, [customizations, editableStr]);

  const theme = useTheme();

  const handleFontSizeChange = (val) => {
    setCustomizations({ ...customizations, fontSize: val, name: editableStr });
  };
  ...
  return (
    <>
      ...
      >
        ...
        <div style={{ fontSize: customizations.fontSize }}>
          <p>
